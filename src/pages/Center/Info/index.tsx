...
function Info(props: Props): ReactElement {
  ...
  const realname = useChange('')
  const major = useChange('')
  const graYear = useChange('')
  const job = useChange('')
  const college = useChange('')
  ...
  const userInfoR = useRequest(getUserInfo, {
    onSuccess: (result, param) => {
      if (result.data) {
        ...
        ////////////////////////////////////////////////////////////////
        realname.setValue(result.data.info.realname)
        graYear.setValue(result.data.info.phone)
        major.setValue(result.data.info.phone)
        job.setValue(result.data.info.phone)
        college.setValue(result.data.info.phone)
    ...
    <Spin spinning={loadStatus}>
      <form className={style.container} onSubmit={handleSubmit}>
        ...
        <div className={style.pwd}>
          ...
          <h2 onClick={() => setDetailInfo(true)}>完善我的详细信息</h2>
        ...
        >
          <form className={style.fix} onInput={checkValidate.run}>
            ...
            {/* <DetailInfo role={role} /> */}
            {(role == '嘉宾') && (
              <>
                <Input type='text' bind={realname} name='姓名' msg='修改需要源手机认证' readOnly update={true} />
                <Input type='text' bind={job} name='职位' msg='修改需要源手机认证' readOnly update={true} />
                <Input type='text' bind={college} name='学院' msg='修改需要源手机认证' readOnly update={true} />
              </>
            )}
            {(role == '学生') && (
              <>
